...
def _freeze_signature(pins: Mapping[str, Any]) -> dict[str, Any]:
    keys = (
        "design", "instrument", "rq_ids", "core_method", "protocol",
        "research_snapshot", "project_config_digest", "effective_profile_set_digest",
        "virtual_runner_descriptor", "runner_digest", "survey_binding_digest",
    )
    return {key: deepcopy(pins.get(key)) for key in keys}


def prior_scenarios(execution_store, prior_run_ids: Sequence[str], *, current_pins: Mapping[str, Any]) -> set[str]:
    scenarios: set[str] = set()
    expected = _freeze_signature(current_pins)
    for run_id in prior_run_ids:
        run = execution_store.load_run(str(run_id))
        if run is None or run.status.value != "COMPLETED" or run.capability_id != "virtual-runner" or run.execution_mode != "virtual":
            raise CapabilityExecutionError("VR-FREEZE-STALE-001", f"prior Virtual Run is missing, incomplete, or incompatible: {run_id}")
        metas = [meta for meta in execution_store.artifacts_for(str(run_id)) if meta.role == "survey_virtual.virtual_runner_result"]
        if len(metas) != 1:
            raise CapabilityExecutionError("VR-FREEZE-STALE-001", f"prior Virtual Run has no exact persisted result: {run_id}")
        try:
            extension = json.loads(execution_store.load_artifact(metas[0].artifact_id).content.decode("utf-8"))
        except Exception as exc:
            raise CapabilityExecutionError("VR-FREEZE-STALE-001", f"prior Virtual Run result is unreadable: {run_id}") from exc
        if not isinstance(extension, Mapping) or _freeze_signature(extension.get("input_pins", {})) != expected:
            raise CapabilityExecutionError("VR-FREEZE-STALE-001", f"prior Virtual Run input pins do not match the current freeze: {run_id}")
        result = extension.get("virtual_runner_result")
        if not isinstance(result, Mapping) or result.get("scenario_class") not in {"STANDARD", "STRESS"}:
            raise CapabilityExecutionError("VR-FREEZE-STALE-001", f"prior Virtual Run scenario is invalid: {run_id}")
...
def readiness_assessment(*, scenario_class: str, prior: set[str], defects: Sequence[Mapping[str, Any]], policy: Mapping[str, Any]) -> dict[str, Any]:
    ...
    open_blocking = [item for item in defects if item.get("disposition") == "open" and item.get("severity") in blocking]
    if open_blocking:
        reasons.append("open blocking Virtual Runner defects remain")
    ready = not reasons
    if ready:
        reasons.append("required virtual scenarios completed under matching pins with no open blocking defects")
    return {
