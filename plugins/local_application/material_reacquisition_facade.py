...
class HistoricalMaterialReacquisitionService:
    ...
    def _previous_success(self, historical_run_id: str, capture_id: str, kind: str):
        for child in reversed(child_runs_for_parent(self._store, historical_run_id, limit=100)):
            ...
            if not isinstance(target, Mapping):
                continue
            if target.get("capture_id") != capture_id or target.get("kind") != kind:
                continue
    ...
    def reacquire(self, historical_run_id: str, capture_id: str, *, kind: str):
        ...
        previous = self._previous_success(historical_run_id, capture_id, kind)
        if previous is not None:
            child, payload = previous
            return {**payload, "reacquisition_run_id": child.run_id, "idempotent_reuse": True}
        ...
        max_bytes = max(int(self._store.config.max_artifact_bytes), int(artifact.size))
        ...
        try:
            ...
            if not isinstance(retrieved, RetrievedMaterial):
                raise MaterialReacquisitionRetrievalError("retriever returned invalid material")
            ...
            if _historical_equivalent(
                ...
                restored = self._restore_identical(artifact.artifact_id, retrieved.content)
                payload = {
                    **common,
