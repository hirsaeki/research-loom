    ...
    def test_shared_blob_loss_and_exact_recovery_keep_history_truthful(self):
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            try:
                ...
                source = facade._workspace_root / "recovery-source.bin"
                source.write_bytes(original_bytes)
                recovered = HistoricalMaterialRecoveryService(store, facade.project_id, facade._workspace_root).recover(
                    runs[0].run_id,
                    "CAP-0",
                    kind="original",
                    source_file=source,
                ...
                restored = facade.list_external_materials()["materials"][0]
                self.assertEqual(restored["backing_content_status"], "verified")
                self.assertEqual(restored["verified_backing_copy_count"], 1)
                self.assertEqual(
                    [(item["run_id"], item["capture_id"], item["captured_at"]) for item in restored["captures"]],
                    history,
                )
            finally:
                facade.close()
