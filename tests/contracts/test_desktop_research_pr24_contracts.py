...
class DesktopResearchPR24ContractExtensions(unittest.TestCase):
    def test_attempt_failure_outcome_and_actual_capture_budget_fields_are_canonical(self):
        ...
        outcomes = set(
            result["$defs"]["search_entry"]["properties"]["outcome"]["enum"]
        )
    ...
    def test_semantics_keep_attempt_ledger_operational_and_quality_external(self):
        ...
        self.assertTrue(
            semantics["search_trace"]["production_attempt_ledger_required"]
        )
        self.assertFalse(
            semantics["search_trace"]["acquisition_failure_is_absence_evidence"]
        )
        self.assertFalse(
            semantics["coverage_and_stopping"]
            ["operational_termination_is_research_stopping"]
        )
        self.assertEqual(
            semantics["quality_boundary"]["source_quality_owned_by"],
            "research_profile",
        )
