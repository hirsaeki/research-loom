...
def binding(proposal: dict[str, Any]) -> dict[str, str]:
    return {"proposal_id": proposal["proposal_id"], "proposal_digest": proposal["proposal_digest"]}


def action_binding(proposal: dict[str, Any]) -> dict[str, str]:
    return {"action_type": proposal["action"]["action_type"], "payload_digest": proposal["action"]["payload_digest"]}
...
def _duplicates(flow):
    seen = set()
    for document in flow["documents"]:
        key = (document["message_type"], document[ID_FIELDS[document["message_type"]]])
        if key in seen:
            return True
        seen.add(key)
    return False
...
def _candidate(handoff, kind, candidate_id):
    key = "candidate_next_actions" if kind == "next_action" else "candidate_next_methods"
    return next((item for item in handoff["outputs"][key] if item["proposal_id"] == candidate_id), None)


def _proposal_for_source(index, input_id):
    return [proposal for proposal in index.get("action_proposal", {}).values() if proposal["source"].get("source_type") == "human_input" and proposal["source"].get("input_id") == input_id]


def _research_binding_from_invocation(invocation):
    return {
        "project_config_digest": invocation["pins"]["project_config"]["configuration_digest"],
        "effective_profile_set_digest": invocation["pins"]["effective_profile_set"]["content_digest"],
...
def conversation_semantic_error(flow, invocation, handoff):
    ...
    for document in flow["documents"]:
        if document[DIGEST_FIELDS[document["message_type"]]] != document_digest(document):
            return {
        ...
        if document["message_type"] == "action_proposal" and document["action"]["payload_digest"] != payload_digest(document):
    ...
    for proposal in proposals.values():
        ...
        if source["source_type"] == "human_input":
            ...
            if input_document["project_id"] != proposal["project_id"] or input_document["conversation_id"] != proposal["conversation_id"] or input_document["actor"] != proposal["initiating_actor"]:
                return "CONV-NATURAL-LANGUAGE-AUTHORITY-001"
            ...
            if proposal["commitment_mode"] != "proposal_only" or proposal["route"]["route_type"] != "unresolved":
                return "CONV-PROPOSAL-ONLY-001"
            if source["candidate_kind"] == "next_method" and not proposal["human_decision_boundary"]["required"]:
                return "CONV-HUMAN-DECISION-001"

        if proposal["commitment_mode"] == "commit_requested" and proposal["route"]["route_type"] == "unresolved":
    ...
    for proposal in proposals.values():
        ...
        if not _same(route["capability"], invocation["capability"]):
            return "CONV-PIN-001"
        if route["execution_mode"] != invocation["execution_mode"] or not _same(route["context_pack"], invocation["context_pack"]):
            return "CONV-PIN-001"
    ...
    for request in requests.values():
        ...
        if proposal is None or not _same(request["proposal_binding"], binding(proposal)):
            return "CONV-CONFIRMATION-BINDING-001"
        if proposal["action"]["effect"] != "state_changing" or proposal["commitment_mode"] != "commit_requested":
            return "CONV-CONFIRMATION-BINDING-001"
        if request["actor_binding"]["actor_type"] != "human" or request["actor_binding"] != proposal["initiating_actor"]:
            return "CONV-CONFIRMATION-BINDING-001"
        if not _same(request["action_binding"], action_binding(proposal)) or not _same(request["state_binding"], proposal["bindings"]["current_state"]):
            return "CONV-CONFIRMATION-BINDING-001"
    ...
    for confirmation in confirmations.values():
        ...
        if request is None or confirmation["request_binding"]["request_digest"] != request["request_digest"]:
            return "CONV-CONFIRMATION-BINDING-001"
        proposal = proposals.get(confirmation["proposal_binding"]["proposal_id"])
        if proposal is None or not _same(confirmation["proposal_binding"], request["proposal_binding"]) or not _same(confirmation["proposal_binding"], binding(proposal)):
            return "CONV-CONFIRMATION-BINDING-001"
        if confirmation["actor"]["actor_type"] != "human" or confirmation["actor"] != request["actor_binding"]:
            return "CONV-CONFIRMATION-BINDING-001"
        if not _same(confirmation["action_binding"], request["action_binding"]) or not _same(confirmation["observed_state"], request["state_binding"]):
            return "CONV-CONFIRMATION-BINDING-001"
        if request.get("research_context_binding") is None:
            if confirmation.get("research_context_binding") is not None:
                return "CONV-CONFIRMATION-BINDING-001"
        elif not _same(confirmation.get("research_context_binding"), request["research_context_binding"]):
            return "CONV-CONFIRMATION-BINDING-001"
        if _parse_time(confirmation["confirmed_at"]) >= _parse_time(request["expires_at"]):
            return "CONV-CONFIRMATION-EXPIRED-001"
        matching_inputs = [item for item in inputs.values() if item["classification"] == "CONFIRMATION" and item["target"]["target_id"] == request_id and item["actor"] == confirmation["actor"]]
    ...
    for receipt in receipts.values():
        ...
        if proposal is None or not _same(receipt["proposal_binding"], binding(proposal)):
            return "CONV-AUDIT-001"
        if not _same(receipt["action_binding"], action_binding(proposal)) or receipt["effect"] != proposal["action"]["effect"]:
            return "CONV-AUDIT-001"
        if receipt["effect"] == "read_only":
            if receipt.get("confirmation_receipt_binding") is not None or receipt["research_state_mutation_performed"] or not _same(receipt["state_before"], receipt["state_after"]):
                return "CONV-READONLY-001"
        if receipt["status"] == "cancelled":
            source = inputs.get(receipt["source_input_id"])
            if source is None or source["classification"] != "CANCEL" or source["target"]["target_type"] != "proposal" or source["target"]["target_id"] != proposal["proposal_id"]:
                return "CONV-CANCEL-001"
            if proposal["commitment_mode"] != "proposal_only" or receipt["execution"]["execution_type"] != "none" or receipt["research_state_mutation_performed"] or not _same(receipt["state_before"], receipt["state_after"]):
                return "CONV-CANCEL-001"
        ...
        if receipt["effect"] == "state_changing" and receipt["status"] == "succeeded":
            ...
            if confirmation_binding is None:
                return "CONV-AUDIT-001"
            confirmation = confirmations.get(confirmation_binding["confirmation_receipt_id"])
            if confirmation is None or confirmation["receipt_digest"] != confirmation_binding["receipt_digest"]:
                return "CONV-AUDIT-001"
            request = requests.get(confirmation["request_binding"]["confirmation_request_id"])
            if request is None or not _same(receipt["state_before"], request["state_binding"]) or receipt.get("research_context_binding") != request.get("research_context_binding"):
                return "CONV-AUDIT-001"

        route = proposal["route"]
        execution = receipt["execution"]
        if receipt["status"] == "succeeded" and route["route_type"] == "harness_service":
            if execution["execution_type"] != "harness_service" or execution["service_id"] != route["service_id"]:
                return "CONV-AUDIT-001"
        if receipt["status"] == "succeeded" and route["route_type"] == "capability_invocation":
            if execution["execution_type"] != "capability_invocation" or execution["invocation_contract"] != "capability-invocation@0.1.0":
                return "CONV-ROUTE-001"
            if execution["invocation_id"] != invocation["invocation_id"] or execution["invocation_digest"] != invocation["invocation_digest"]:
                return "CONV-ROUTE-001"
            if "handoff" in execution and (execution["handoff"]["handoff_id"] != handoff["handoff_id"] or execution["handoff"]["handoff_digest"] != handoff["handoff_digest"]):
                return "CONV-ROUTE-001"
    ...
    for presentation in presentations.values():
        ...
        if handoff_binding["handoff_id"] != handoff["handoff_id"] or handoff_binding["handoff_digest"] != handoff["handoff_digest"] or handoff_binding["invocation_id"] != handoff["invocation_id"]:
            return "CONV-HANDOFF-PRESENTATION-001"
        candidate = presentation["candidate"]
        if _candidate(handoff, candidate["candidate_kind"], candidate["candidate_proposal_id"]) is None:
            return "CONV-HANDOFF-PRESENTATION-001"
        proposal = proposals.get(presentation["proposal_binding"]["proposal_id"])
        if proposal is None or not _same(presentation["proposal_binding"], binding(proposal)):
            return "CONV-HANDOFF-PRESENTATION-001"
        source = proposal["source"]
        if source["source_type"] != "capability_handoff_candidate" or source["handoff_id"] != handoff["handoff_id"] or source["handoff_digest"] != handoff["handoff_digest"] or source["candidate_kind"] != candidate["candidate_kind"] or source["candidate_proposal_id"] != candidate["candidate_proposal_id"]:
            return "CONV-HANDOFF-PRESENTATION-001"
        if proposal["commitment_mode"] != "proposal_only" or proposal["route"]["route_type"] != "unresolved":
            return "CONV-PROPOSAL-ONLY-001"
        if candidate["candidate_kind"] == "next_method" and not proposal["human_decision_boundary"]["required"]:
            return "CONV-HUMAN-DECISION-001"
...
def apply_semantic_case(flow, case):
    ...
    target = next(document for document in result["documents"] if document["message_type"] == message_type and document[ID_FIELDS[message_type]] == target_id)
    op = case["mutation"]["op"]
    if op == "set":
        cursor = target
        ...
        duplicate[ID_FIELDS[message_type]] = case["mutation"].get("new_id", target_id)
        if message_type == "confirmation_receipt" and "new_id" in case["mutation"]:
            duplicate["confirmation_receipt_id"] = case["mutation"]["new_id"]
