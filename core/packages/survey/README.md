...
Issue #221 adds the first production REAL response intake over that same boundary. A controlled workspace-local provider-neutral JSON file is bound to one exact Instrument revision, normalized into the existing immutable Dataset, aggregated by the existing shared analysis service, and exposed through joined read-only inspection. REAL intake remains `EMPIRICAL`; synthetic identities are rejected from the valid REAL population; aggregate interpretation remains candidate research material and never becomes a Finding automatically.
...
Out of scope: provider-specific Forms/Qualtrics connectors, OAuth, contact distribution, LLM-backed respondent generation, regression, factor analysis, significance testing, weighting, imputation, causal analysis, psychometrics, NLP interpretation, charts/dashboard work, Delphi, Case Study, and Writer/Publication.
