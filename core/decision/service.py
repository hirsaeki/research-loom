...
from core.runtime import (
    Actor, CommitReceipt, StateTransitionRejected, StateTransitionRequest,
    TransitionAction, TransitionKind, canonical_digest,
)
...
def _requirement_key(requirement: DecisionRequirement) -> tuple[str, str, str, str]:
    return (
        requirement.decision_kind, requirement.choice,
        requirement.subject_kind, requirement.subject_id,
    )


class HumanDecisionService:
    """Production Human Decision Gate using PR20 DecisionRequirement as source of truth."""

    def __init__(self, *, store, state_provider, state_transition_service, clock) -> None:
        self._store = store
    ...
    def gate_candidate(
        ...
        *,
        ...
        actions = self._candidate_actions(candidate)
        requirements = tuple(
            (index, requirement)
        ...
        if not requirements:
            return DecisionGateResult(
                "READY_TO_COMMIT",
                transition_request=self._build_transition_request(
                    state=state, actor=actor, target_actions=actions,
                    ...
                    basis_id=str(candidate["proposal_id"]), decision_actions=(),
                ),
            )
        request = self._build_request(
            candidate, state=state, actor=actor, actions=actions,
            requirements=requirements, source_action_proposal=source_action_proposal,
        )
    ...
    def resolve(self, response: Mapping[str, Any]) -> DecisionResolutionResult:
        ...
        status = self._store.get_status(request_id)
        if status in _TERMINAL:
            raise HumanDecisionError("DECISION-TERMINAL-001", f"Human Decision Request is already {status}")
        ...
        if stale_reason is not None:
            ...
            self._store.finalize(
                request_id, str(response["response_digest"]), "STALE", detail=stale_reason
            )
        ...
        decision_actions, bound_actions = self._materialize_decisions_and_bind(
            request, target_actions,
            actor_id=str(actor["actor_id"]), decided_at=str(response["responded_at"]),
        )
        transition = self._build_transition_request(
            state=current_state, actor=actor, target_actions=bound_actions,
            source_refs=tuple(str(item) for item in request.get("source_refs", ())),
            authorization_evidence=(), basis_id=request_id, decision_actions=decision_actions,
        )
        result = self._transitions.apply(transition)
        if isinstance(result, CommitReceipt):
            self._store.finalize(
                request_id, str(response["response_digest"]), "RESOLVED",
        ...
        head_codes = {"RT-HEAD-001", "RT-HEAD-002", "RT-PIN-002", "RT-PIN-003", "RT-PIN-004"}
        if any(issue.error_code in head_codes for issue in result.issues):
            self._store.finalize(
                request_id, str(response["response_digest"]), "STALE",
    ...
    def _claim_or_raise(self, request, response) -> None:
        claimed = self._store.claim_response(
            str(request["request_id"]), str(request["request_digest"]), response
        )
    ...
    def _validate_candidate(self, candidate, state) -> None:
        required = (
            "proposal_id", "proposal_digest", "project_ref", "lineage_ref",
            "current_snapshot_ref", "current_snapshot_digest",
        )
        missing = [key for key in required if not candidate.get(key)]
        if missing:
            raise HumanDecisionError("DECISION-CANDIDATE-001", f"candidate missing binding fields: {', '.join(missing)}")
        basis = deepcopy(dict(candidate)); supplied = str(basis.pop("proposal_digest", ""))
    ...
    def _candidate_actions(self, candidate) -> tuple[TransitionAction, ...]:
        actions = []
        for raw in candidate.get("proposed_actions", ()):
            if raw.get("decision_refs"):
                raise HumanDecisionError(
                    "DECISION-FORGED-REF-001", "candidate actions may not smuggle Human Decision refs"
                )
    ...
    def _build_request(self, candidate, *, state, actor, actions, requirements, source_action_proposal):
        requirement_basis = [
            {
                "action_index": index, "action_digest": _action_digest(actions[index]),
                "decision_kind": requirement.decision_kind, "choice": requirement.choice,
                "subject_kind": requirement.subject_kind, "subject_id": requirement.subject_id,
            }
        ...
        request_id = _stable_id(
            "HDREQ-", str(candidate["proposal_id"]), str(candidate["proposal_digest"]),
            str(state.current_snapshot["content_digest"]), requirement_set_digest,
        )
        units = []
        ...
        return with_request_digest({
            "schema_version": "0.1.0", "request_id": request_id,
            "project_ref": state.project_ref, "lineage_ref": state.lineage_ref,
            "human_actor_id": str(actor["actor_id"]),
            "source_state_delta_proposal": {
                "proposal_id": str(candidate["proposal_id"]), "proposal_digest": str(candidate["proposal_digest"]),
            },
            ...
            "effective_profile_set": {"ref": state.effective_profile_set_ref, "digest": state.effective_profile_set_digest},
            ...
            "issued_at": self._clock.now(), "status": "PENDING",
        })

    def _validate_response_envelope(self, response) -> None:
        required = {"request_id", "request_digest", "disposition", "actor", "responded_at", "response_digest"}
    ...
    def _stale_reason(self, request, state) -> str | None:
        ...
        if request["project_config"]["ref"] != state.project_config_ref or request["project_config"]["digest"] != state.project_config_digest:
            return "Project Config binding changed after Decision Request issuance"
        if request["effective_profile_set"]["ref"] != state.effective_profile_set_ref or request["effective_profile_set"]["digest"] != state.effective_profile_set_digest:
        ...
        for index, action in enumerate(self._request_actions(request)):
            for requirement in required_decisions_for_action(state, action):
                rebuilt.append({
                    "action_index": index, "action_digest": _action_digest(action),
                    "decision_kind": requirement.decision_kind, "choice": requirement.choice,
                    "subject_kind": requirement.subject_kind, "subject_id": requirement.subject_id,
                })
    ...
    def _request_actions(self, request) -> tuple[TransitionAction, ...]:
        actions = tuple(TransitionAction(
            kind=TransitionKind(str(raw["kind"])), payload=deepcopy(dict(raw.get("payload", {}))),
        ...
        for unit in request["decision_units"]:
            ...
            if _action_digest(actions[index]) != unit["transition_action_digest"]:
    ...
    def _materialize_decisions_and_bind(self, request, target_actions, *, actor_id, decided_at):
        ...
        for unit in request["decision_units"]:
            ...
            if subject not in grouped.setdefault(key, []): grouped[key].append(subject)
        ...
        for key in sorted(grouped):
            ...
            decision = {
                "schema_version": "0.1.0", "id": decision_id, "kind": "decision", "revision": 0,
                "project_id": str(request["project_ref"]), "decision_kind": key[0],
                "subjects": sorted(grouped[key], key=lambda item: (item["kind"], item["id"])),
                "choice": key[1], "actor_type": "human", "decided_by": actor_id, "decided_at": decided_at,
            }
        ...
        for index, action in enumerate(target_actions):
            ...
            payload = deepcopy(dict(action.payload)); obj = action.object_payload()
            if obj is not None and refs:
                bound_obj = deepcopy(dict(obj))
                existing = tuple(str(item) for item in bound_obj.get("decision_ids", ()))
                if any(item not in refs for item in existing):
                    raise HumanDecisionError("DECISION-FORGED-REF-001", "target object contains unrelated Decision provenance")
                bound_obj["decision_ids"] = list(refs); payload["object"] = bound_obj
                ...
                for treatment in payload.get("treatments", ()):
                    ...
                    if item.get("treatment") == "RECONFIRM":
                        ...
                        if isinstance(derived, Mapping):
                            ...
                            if len(matching) != 1:
                                raise HumanDecisionError("DECISION-BINDING-001", "RECONFIRM requires one exact Decision")
                            item["human_decision_ref"] = matching[0]
                            derived_copy = deepcopy(dict(derived)); derived_copy["decision_ids"] = [matching[0]]
    ...
    def _build_transition_request(
        self, *, state, actor, target_actions, source_refs,
        authorization_evidence, basis_id, decision_actions,
    ) -> StateTransitionRequest:
        request = StateTransitionRequest(
            transition_id=_stable_id("TR-", basis_id), project_ref=state.project_ref,
            ...
            project_config_ref=state.project_config_ref, project_config_digest=state.project_config_digest,
            effective_profile_set_ref=state.effective_profile_set_ref,
            effective_profile_set_digest=state.effective_profile_set_digest,
            authorization_evidence=tuple(str(item) for item in authorization_evidence),
            idempotency_key=_stable_id("IDEMP-HD-", basis_id), submitted_at=self._clock.now(),
            new_snapshot_id=_stable_id("SNP-HD-", basis_id), commit_id=_stable_id("COM-HD-", basis_id),
            audit_event_id=_stable_id("AUD-HD-", basis_id), source_refs=tuple(source_refs),
        )
        return request.with_calculated_digest()


def make_response(*, request: Mapping[str, Any], disposition: str, actor_id: str, responded_at: str) -> Mapping[str, Any]:
    """Build explicit structured Human response; this function never parses prose."""
    return with_response_digest({
        ...
        "disposition": disposition, "actor": {"actor_id": actor_id, "actor_type": "human"},
