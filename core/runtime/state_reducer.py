...
def reduce_state(current_state: StateView, request: StateTransitionRequest) -> ReductionResult:
    ...
    This function performs no I/O, obtains no wall-clock time, allocates no IDs,
    and dispatches only on the closed canonical TransitionKind vocabulary. All
    identity/time values needed to build immutable objects are supplied by the
    StateTransitionRequest.
    """

    lineage_actions = [action for action in request.actions if action.kind == TransitionKind.APPLY_LINEAGE_PLAN]
    ...
    if lineage_actions:
        if len(request.actions) != 1:
            raise ReductionError("APPLY_LINEAGE_PLAN cannot be mixed with other state actions")
        return _reduce_lineage_plan(current_state, request, lineage_actions[0])
    ...
    state_changed = False
    active_lineage_update: str | None = None

    for action in request.actions:
        if action.kind == TransitionKind.SWITCH_ACTIVE_LINEAGE:
            if len(request.actions) != 1:
                raise ReductionError("SWITCH_ACTIVE_LINEAGE cannot be mixed with other state actions")
            active_lineage_update = str(action.payload.get("target_lineage_ref", ""))
            if not active_lineage_update:
    ...
    if active_lineage_update is not None:
        return ReductionResult(
            object_revisions=(), decision_records=(), new_snapshot=None,
            lineage_updates=(), new_lineages=(), active_lineage_update=active_lineage_update,
            adoption_refs=(), used_decision_refs=used_decisions, audit_events=(audit,),
    ...
    snapshot = _build_snapshot(
        current_state=current_state, request=request, members=effective,
        prior_snapshot_id=str(current_state.current_snapshot["id"]), mode=current_state.current_snapshot_ref.mode,
    )
    ...
    updated_line = replace(
        line, head_snapshot_ref=str(snapshot["id"]),
        head_snapshot_digest=str(snapshot["content_digest"]), head_snapshot_revision=int(snapshot["revision"]),
    )

    return ReductionResult(
        object_revisions=stable_sorted_objects(new_revisions),
        decision_records=stable_sorted_objects(decisions), new_snapshot=snapshot,
        lineage_updates=(updated_line,), new_lineages=(), active_lineage_update=None,
        adoption_refs=tuple(dict.fromkeys(adoption_refs)), used_decision_refs=used_decisions,
        audit_events=(audit,), applied_actions=tuple(action.kind.value for action in request.actions),
    )


def _reduce_lineage_plan(current_state: StateView, request: StateTransitionRequest, action: TransitionAction) -> ReductionResult:
    ...
    snapshot = _build_snapshot(
        current_state=current_state, request=request, members=child_members,
        prior_snapshot_id=str(current_state.current_snapshot["id"]), mode=execution_mode,
    )
    ...
    child = LineageView(
        ...
        project_config_ref=request.project_config_ref, project_config_digest=request.project_config_digest,
    ...
    audit = _audit_event(current_state, request)
    return ReductionResult(
        object_revisions=stable_sorted_objects(new_revisions), decision_records=(), new_snapshot=snapshot,
        lineage_updates=(), new_lineages=(child,), active_lineage_update=None,
        adoption_refs=tuple(
            ref for ref in (
        ...
        used_decision_refs=resolving_decision_refs(current_state, request), audit_events=(audit,),
...
def _validate_revision_semantics(action: TransitionAction, obj: Mapping[str, Any], prior: Mapping[str, Any] | None) -> None:
...
def _build_snapshot(*, current_state: StateView, request: StateTransitionRequest, members: Mapping[tuple[str, str], Mapping[str, Any]], prior_snapshot_id: str, mode: str) -> Mapping[str, Any]:
    member_rows = [
    ...
    payload: dict[str, Any] = {
        "schema_version": "0.1.0", "id": request.new_snapshot_id, "kind": "snapshot", "revision": 0,
        "project_id": request.project_ref, "snapshot_type": "research", "created_at": request.submitted_at,
        "mode": mode, "prior_snapshot_id": prior_snapshot_id, "members": member_rows,
    }
...
def _audit_event(current_state: StateView, request: StateTransitionRequest) -> Mapping[str, Any]:
    ...
    for action in request.actions:
        ...
        if target is not None:
            ...
            subjects.append({"kind": "research_lineage", "id": str(action.payload.get("target_lineage_id", request.lineage_ref))})
        elif action.kind == TransitionKind.SWITCH_ACTIVE_LINEAGE:
            subjects.append({"kind": "research_lineage", "id": str(action.payload.get("target_lineage_ref", request.lineage_ref))})
        else:
    ...
    return {
        "schema_version": "0.1.0", "id": request.audit_event_id, "kind": "audit_event", "revision": 0,
        "project_id": request.project_ref, "occurred_at": request.submitted_at, "actor_type": actor_type,
        "actor_id": request.actor.actor_id, "action": "+".join(action.kind.value for action in request.actions),
        "subjects": _dedupe_subjects(subjects), "payload_digest": request.request_digest,
    }
