...
from .ports import ResourceProvider
...
class BoundedResourceAccess:
    ...
    def __init__(self, context_pack: Mapping[str, Any], authorized_reference_ids: tuple[str, ...], provider: ResourceProvider) -> None:
        self._resources = {str(item["reference_id"]): dict(item) for item in context_pack["resources"]}
    ...
    def read(self, reference_id: str) -> ResourcePayload:
        ...
        if resource is None or reference_id not in self._authorized:
            raise CapabilityExecutionError(ExecutionFailureCode.RESOURCE_DENIED, f"resource {reference_id!r} is outside the bounded authorized Context Pack")
