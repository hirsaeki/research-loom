...
Capture multiplicity is logical provenance, not physical redundancy. For the bounded material page being returned, the inventory verifies each distinct content-addressed backing object with the same digest/size rules used by verified material reads. Material output therefore reports the logical `capture_count` separately from `backing_content_status`, `content_available`, and `verified_backing_copy_count` (currently `0` or `1` for the single-CAS implementation). Original and UTF-8 rendition health are projected independently on each capture. A missing or corrupt backing blob does not remove or rewrite its historical capture row, and managed filesystem paths are never exposed. Health is operational storage state only; it does not alter Research State, adoption, or historical acquisition metadata.
