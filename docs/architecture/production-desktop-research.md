...
Capture multiplicity is logical provenance, not physical redundancy. For the bounded material page being returned, the inventory verifies each material's original content-addressed backing object with the same digest/size rules used by verified material reads. Material output therefore reports the logical `capture_count` separately from `backing_content_status`, `content_available`, and `verified_backing_copy_count` (currently `0` or `1` for the single-CAS implementation). Original and UTF-8 rendition health are projected independently without turning list into an unbounded historical rehash. Each listed material verifies its single-CAS original backing and one representative UTF-8 rendition; later captures that reuse either checked backing reuse that result, while distinct historical renditions carry `verification_performed=false`, `status=null`, and `content_available=null` rather than being guessed healthy. Operators can use exact capture `show` / `export` for a verified read of a specific historical rendition. A missing or corrupt backing blob does not remove or rewrite its historical capture row, and managed filesystem paths are never exposed. Health is operational storage state only; it does not alter Research State, adoption, or historical acquisition metadata.
