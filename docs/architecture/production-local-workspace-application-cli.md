...
The first concrete facade adapter is a structured CLI, not a REPL/TUI:
...
research-loom action submit --workspace PATH --json -
research-loom confirmation submit --workspace PATH --json -
research-loom decision resolve --workspace PATH --json -
research-loom external collect --workspace PATH --run-id RUN-ID --json -
```

`python -m plugins.local_application.cli ...` is equivalent when the repository script is not on `PATH`.
